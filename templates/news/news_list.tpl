{include file="$templates_path/news/horizontal_menu.tpl"}

<h1>{#lang_news_module_title#}</h1>

<div class="entries_list news_list">
{foreach from=$news_list key=news_id item=news_info name=itemslistloop}
	
	{if $news_info.name ne "" || $news_info.short_text ne ""}

		<div class="entry news_item clearfix {if $news_info.news_image.FOTO_ID ne ""}item_with_image{/if}">

			{if $news_info.news_image.FOTO_ID ne ""}
			<div class="entry_image news_picture"><a href="{$base_path}{$news_info.NODE_PATH}{$news_id}/"><img src="{$farch_foto_params.folders.link}/{$news_info.news_image.ALBUM_ID}/{$farch_foto_params.previews.card.prefix}{$news_info.news_image.FOTO_ID}.{$news_info.news_image.TECH_INFO.extension}" alt="{$news_info.name|escape:'html'}"></a></div>
			{/if}
			...
			<div class="entry_date news_date">
